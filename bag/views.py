...
def add_to_bag(request, item_id):
    """ add a product and its selected quantity to the bag """
    ...
    print(f'in add to bag: {bag}')

    return redirect(redirect_url)


def update_bag(request, item_id):
    """ update the quantity of the selected product to the udated quantity """
...
def remove_item(request, item_id):
    """ remove the product formt he shoppign bag """
    try:
        product = get_object_or_404(Product, pk=item_id)
        size = None
        ...
        message.error(request, f'Error when removing item {e} from your bag')
